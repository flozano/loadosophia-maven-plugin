...
/**
 * This comes from jmeter plugins with Apache 2.0 license
 * 
...
public class LoadosophiaAPIClient {
	...
	private static final Logger log = LoggerFactory
			.getLogger(LoadosophiaAPIClient.class);
	...
	public LoadosophiaAPIClient(StatusNotifierCallback informer,
	...
	public LoadosophiaUploadResults sendFiles(File targetFile,
			LinkedList<String> perfMonFiles) throws IOException {
		...
		partsList.add(new FilePart("jtl_file", new FilePartSource(
				gzipFile(targetFile))));
		...
		while (it.hasNext()) {
			...
			if (!perfmonFile.exists()) {
				log.warn("File not exists, skipped: "
						+ perfmonFile.getAbsolutePath());
				continue;
			}
			partsList.add(new FilePart("perfmon_" + index, new FilePartSource(
					gzipFile(perfmonFile))));
			perfmonFile.delete();
			index++;
		}

		notifier.notifyAbout("Starting upload to Loadosophia.org");
		String[] fields = multipartPost(partsList, getUploaderURI(),
				HttpStatus.SC_OK);
		...
		if (!title.trim().isEmpty() || !colorFlag.equals(COLOR_NONE)) {
			...
			results.setRedirectLink(address + "api/file/status/" + queueID
					+ "/?redirect=true");
		}
		return results;
	}
	...
	private int getTestByUpload(int queueID) throws IOException {
		while (true) {
			...
			if (status.length > 2 && !status[2].isEmpty()) {
				throw new RuntimeException("Loadosophia processing error: "
						+ status[2]);
			}
	...
	protected String[] multipartPost(LinkedList<Part> parts, String URL,
			int expectedSC) throws IOException {
